...
\begin{itemize}
	...
	\item four floats $s_l, \alpha_l, s_r, \alpha_r$ specifying the size and angle of the slopes left and right of the curve
\end{itemize}
...
\begin{equation}
	H^{k+1}[i,j] = \alpha E[i,j] + \beta G^{k+1}[i,j] + (1-\alpha-\beta) L^{k+1}[i,j]
\label{eq:Diffusion1}
\end{equation}
...
\label{eq:Diffusion3}
\end{equation}
The gradient equation tend to satisfy the provided gradient $G$. $G$ is directly calculated from $\sin(\alpha_l)$ and $\sin(\alpha_r)$, i.e. from the slope angles defined at the control points.
