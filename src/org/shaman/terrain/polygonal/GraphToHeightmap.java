...
public class GraphToHeightmap {
	...
	static {
		...
		BIOME_PROPERTIES.put(Biome.TUNDRA, new double[]{0.5, 0.5, 0.3, 0.5});
		BIOME_PROPERTIES.put(Biome.BARE, new double[]{0.4, 0.4, 0.3, 0.5});
		...
		BIOME_PROPERTIES.put(Biome.TEMPERATE_DESERT, new double[]{0.1, 0.1, 0.4, 0});
		...
		BIOME_PROPERTIES.put(Biome.TROPICAL_RAIN_FOREST, new double[]{0.3, 0.2, 0.1, 0});
		BIOME_PROPERTIES.put(Biome.TROPICAL_SEASONAL_FOREST, new double[]{0.3, 0.2, 0, 0});
		BIOME_PROPERTIES.put(Biome.SUBTROPICAL_DESERT, new double[]{0.3, 0.6, 0.5, 0});
		BIOME_PROPERTIES.put(Biome.BEACH, new double[]{0.3, 0.5, 0.4, 0});
		BIOME_PROPERTIES.put(Biome.LAKE, new double[]{0.2, 0.2, 0.2, 0});
	...
	private static final float PERLIN_NOISE_SCALE = 0.1f;
	...
	private void calculateElevation() {
		...
		for (int x=0; x<size; ++x) {
			for (int y=0; y<size; ++y) {
				...
				heightmap.adjustHeightAt(x, y, (float) perlin);
			}
		}
	}
	...
	private Geometry createNoiseGeometry() {
		...
		for (Graph.Center c : graph.centers) {
			...
			ColorRGBA col = new ColorRGBA((float) settings[0], (float) settings[1], (float) (settings[2] + settings[3]*c.elevation), 1);
			colorList.add(col);
